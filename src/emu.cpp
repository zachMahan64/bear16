...
#define PRINT_BORDER_DIV_LINE() std::cout << emuBorderDivLine << std::endl
    ...
    PRINT_TITLE_BAR();
    std::cout << std::format("| {:<57} |", "[1] Assemble & Run Without Saving Executable") << std::endl;
...
std::filesystem::path Emulator::computeBinPath() const {
    ...
    if (!std::filesystem::exists(projectFilePath)) {
        std::cerr << "Project file does not exist: " << projectPath << std::endl;
        return {};
    }
    // compute the output binary path
    std::filesystem::path executableName = projectFilePath.stem(); // filename w/o extension
    std::filesystem::path executablePath = projectFilePath / (executableName.string() + ".bin");
