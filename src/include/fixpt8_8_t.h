...
struct fixpt8_8_t {
    ...
    fixpt8_8_t operator*(T rhs) const
        requires(std::is_arithmetic_v<T>)
    {
        int32_t scaled_rhs = std::is_floating_point_v<T> ? static_cast<int32_t>(rhs * 256.0)
    ...
    fixpt8_8_t operator+(const fixpt8_8_t& rhs) const { return fixpt8_8_t(val + rhs.val); }
